...
export default function CooRequisitionApproval() {
  ...
  const filteredRequisitions = useMemo(() => {
    ...
    if (activeTab === "Pending") {
      filtered = requisitions.filter(
        (r) => !r.approvals?.coo?.approval?.status || r.approvals?.coo?.approval?.status === "pending"
      );
    } else if (activeTab === "Approved") {
      filtered = requisitions.filter((r) => r.approvals?.coo?.approval?.status === "approved");
    } else if (activeTab === "Rejected") {
      filtered = requisitions.filter((r) => r.approvals?.coo?.approval?.status === "rejected");
    }
  ...
  }, [activeTab, requisitions, query]);

  useEffect(() => {
    if (selectedReq) {
      const existing = selectedReq.approvals?.coo?.approval?.comments || "";
  ...
  const viewRequisition = async (id) => {
    try {
      ...
      setComments(requisition.approvals?.coo?.approval?.comments || "");
      setShowModal(true);
    } catch (err) {
  ...
  const handleAction = async (id, status) => {
    ...
    setLoading(true);
    try {
      await api.put(`/requisitions/cooApproval/${id}`, { status, comments });
    ...
    <div className="flex h-screen bg-[#F5F5F5]">
      ...
      <main className="flex-1 flex flex-col overflow-auto">
        <ProfileHeader
          ...
          subtitle="Review and approve requisitions."
                    ...
                    <tbody>
                      {filteredRequisitions.length > 0 ? (
                              ...
                              <td className="p-3 text-center">
                                <span
                                  className={`px-3 py-1 rounded-full text-sm font-medium ${
                                    r.approvals?.coo?.approval?.status === "approved"
                                      ? "bg-green-100 text-green-700"
                                      : r.approvals?.coo?.approval?.status === "rejected"
                                      ? "bg-red-100 text-red-700"
                                      : "bg-yellow-100 text-yellow-700"
                                  }`}
                                >
                                  {r.approvals?.coo?.approval?.status || "Pending"}
                                </span>
                              </td>
                              <td className="p-3 text-center">
                                <button
                        ...
                        <tr>
                          ...
                          >
                            No {activeTab.toLowerCase()} requisitions.
                          </td>
                        </tr>
                      )}
                    </tbody>
                  ...
                  <Dialog.Panel className="bg-white rounded-2xl shadow-2xl max-w-5xl w-full overflow-hidden flex flex-col max-h-[90vh] border border-gray-100">
                    <div className="p-6 sm:p-8 overflow-y-auto">
                      ...
                      <div className="flex flex-col sm:flex-row items-start justify-between mb-6">
                        <div>
                          <h2 className="text-2xl font-bold text-gray-900">{selectedReq.position}</h2>
                        ...
                        <div className="text-left sm:text-right mt-4 sm:mt-0 flex-shrink-0">
                          ...
                          <p className="text-sm font-medium text-gray-400">
                            {selectedReq.createdAt ? new Date(selectedReq.createdAt).toISOString().split("T")[0] : "-"}
                          </p>
                        </div>
                      </div>
                        ...
                        <div className="space-y-4">
                          ...
                          {selectedReq.requisitionType === "Replacement" && (
                            <div className="mt-3 p-4 bg-gray-50 rounded-xl border">
                              ...
                              <p><span className="text-gray-500">Designation:</span> {selectedReq.replacementDetail?.desig || "-"}</p>
                              <p><span className="text-gray-500">Reason:</span> {selectedReq.replacementDetail?.reason || "-"}</p>
                              <p><span className="text-gray-500">salary:</span> {selectedReq.replacementDetail?.salary || "-"}</p>
                            </div>
                          )}
                        </div>
                        <div className="space-y-4">
                            ...
                            <InfoBlock
                              ...
                              value={
                                selectedReq.desiredDateJoin
                                  ? new Date(selectedReq.desiredDateJoin).toLocaleDateString("en-GB")
                                  : "-"
                              }
                            />
                            <InfoBlock label="Approved Position" value={selectedReq.approvedPosition || "-"} />
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm text-gray-700">
                        ...
                        <div>
                          ...
                          <p><span className="text-gray-500">Experience:</span> {selectedReq.experience || "-"}</p>
                      ...
                      <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                          <CommentCard
                            ...
                            approver={selectedReq.approvals?.departmentHead?.approval?.reviewer?.name || selectedReq.assignedHod?.name}
                          ...
                          <CommentCard
                            ...
                            approver={selectedReq.approvals?.hr?.approval?.reviewer?.name || "HR"}
                            date={selectedReq.approvals?.hr?.approval?.reviewedAt}
                            comments={selectedReq.approvals?.hr?.approval?.comments}
                          />
                          <CommentCard
                            title="Chief Operating Officer"
                            approver={selectedReq.approvals?.coo?.approval?.reviewer?.name}
                      ...
                      {/* --- FIX: Show actions only on Pending tab --- */}
                      {activeTab === "Pending" && (
                        <>
                          <hr className="my-6 border-gray-200" />
                          <textarea

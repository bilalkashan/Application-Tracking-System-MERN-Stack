...
import { Eye, PlusCircle, Loader2, CheckCircle, XCircle, Hourglass, Trash, Search } from "lucide-react";
...
export default function HrRequisitionApproval() {
  ...
  const filteredRequisitions = useMemo(() => {
    ...
    if (activeTab === "Pending") {
      filtered = requisitions.filter(
        (r) => !r.approvals?.hr?.approval?.status || r.approvals?.hr?.approval?.status === "pending"
      );
    } else if (activeTab === "Approved") {
      filtered = requisitions.filter((r) => r.approvals?.hr?.approval?.status === "approved");
    } else if (activeTab === "Rejected") {
      filtered = requisitions.filter((r) => r.approvals?.hr?.approval?.status === "rejected");
    }
  ...
  useEffect(() => {
    if (selectedReq) {
      const existing = selectedReq.approvals?.hr?.approval?.comments || "";
  ...
  setComments(requisition.approvals?.hr?.approval?.comments || "");
      setShowModal(true);
    } catch (err) {
  ...
  const handleAction = async (id, status) => {
    ...
    setLoading(true);
    try {
  await api.put(`/requisitions/hrApproval/${id}`, { status, comments });
                              ...
                              <td className="p-3 text-center">
                                <span
                                  className={`px-3 py-1 rounded-full text-sm font-medium ${
                                    r.approvals?.hr?.approval?.status === "approved"
                                      ? "bg-green-100 text-green-700"
                                      : r.approvals?.hr?.approval?.status === "rejected"
                                      ? "bg-red-100 text-red-700"
                                      : "bg-yellow-100 text-yellow-700"
                                  }`}
                                >
                                  {r.approvals?.hr?.approval?.status || "Pending"}
                                </span>
                              </td>
                              <td className="p-3 text-center">
                                <button

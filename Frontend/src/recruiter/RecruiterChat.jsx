...
export default function RecruiterChat({ applicationId }) {
  ...
  useEffect(() => {
    ...
    window.dispatchEvent(
      new CustomEvent("app:indicator:clear", {
        detail: { key: `chat:application:${applicationId}` },
      })
    );
  }, [applicationId]);
  ...
  return (
    <div className="flex flex-col h-full bg-[#F5F5F5]">
      <div className="flex-1 overflow-y-auto overflow-x-hidden bg-[#F5F5F5] rounded-lg p-4 space-y-3 shadow-sm">
      ...
      <div className="flex items-end gap-3 mt-3 border-t border-gray-200 pt-3 bg-white sticky bottom-0">
        <textarea
          ...
          rows={1}
          className="flex-1 resize-none border border-gray-400 shadow-md rounded-2xl px-4 py-2 text-sm bg-gray-100 focus:outline-none focus:ring-1 focus:ring-black transition-all max-h-32 overflow-y-hidden"
          style={{ lineHeight: "1.5" }}
        />
        <button
          onClick={sendMessage}
          className="hover:bg-[#e5383b] hover:text-white bg-[#111] text-white shadow-md px-5 py-2 rounded-full shadow font-medium text-sm transition active:scale-95"
        >
          Send
        </button>
      </div>
